...
import bgMobile from "../../../../assets/images/rate-item-bg-m.png";
import check from "../../../../assets/images/svg/Check.svg";

const RateItemMax = ({ type, post, openModalFunc }) => {
  return (
    <Item className="rateItem" type={type}>
      ...
      {(type === "max" || type === "self") && (
        <PriceWrapper type={type}>
          <Price align="center" color={type === "max" ? "white" : "black"}>
            {post.acf.tariff_2_price}
          </Price>
          {(type === "max" || type === "self") && (
            <Installment>
              <Or type={type}>или</Or>
              <InstallmentP color={type === "max" ? `white` : `black`}>
                {post.acf.tariff_2_installment}
                <InfoIcon>
                  <svg
                    width="16"
                    height="16"
      ...
      <BtnWrapper type={type}>
        {type === "free" ? (
          <WhiteBtn link="/">{post.acf.tariff_2_button_text}</WhiteBtn>
        ) : (
...
const Item = styled.div`
  ...
  @media screen and (max-width: 768px) {
    ...
    ${({ type }) =>
      type === "max" && `background: url(${bgMobile}) no-repeat 50% / cover;`};
  }
`;

export default RateItemMax;
